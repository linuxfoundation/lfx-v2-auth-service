...
- After a successful update, the previous primary email is kept reachable. Unless it is already backed by a verified email/OTP identity or a Google login (which already covers the address), it is preserved as a verified, **user-removable** alternate identity (not system-managed) so the user can later unlink it like any other alternate email. A previous primary backed only by a non-Google provider (e.g. LinkedIn) is preserved this way too. Preservation happens before the switch: if the previous primary cannot be preserved, the switch is aborted and the account is left unchanged rather than dropping the old primary
